...
export default function RadioPage() {
  ...
  const fetchShows = async () => {
    try {
      ...
      const items = Array.isArray(data) ? data : (data.data || data.shows || []);
      setShows(items);
    } catch (error) {
  ...
  const filterShows = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((show) =>
        ...
        (show.host?.toLowerCase().includes(q) ?? false) ||
        (show.tags || []).some((tag) => tag.toLowerCase().includes(q))
      );
    }
  ...
  const handlePlayShow = (show: RadioShow) => {
    ...
    // Intégrer votre player audio ici
    console.log('Lecture de:', show.title, '=>', show.audioUrl);
  };

  const isLiveActive = shows.some((s) => s.isLive);
          ...
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            {isLiveActive ? (
              <div className="space-y-4">
                ...
                <h3 className="text-xl font-semibold text-gray-900">Émission en cours</h3>
                <p className="text-gray-600">Une émission est actuellement diffusée en direct</p>
              ...
              <div className="space-y-4">
                ...
                <h3 className="text-xl font-semibold text-gray-700">Aucun direct pour le moment</h3>
                <p className="text-gray-500">Consultez notre programme ou écoutez nos rediffusions</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          {filteredShows.length === 0 ? (
            <div className="bg-white rounded-lg shadow-md p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-700 mb-2">Aucune émission trouvée</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredShows.map((show) => (
                <RadioShowCard
                  ...
                  host={show.host || '—'}
                  date={new Date(show.broadcastDate)}
