...
const sample: Omit<Teacher, "href">[] = [
  {
    id: "t1",
...
export default function TeachersSection({ locale }: Props) {
  const items: Teacher[] = sample.map((t) => ({ ...t, href: `/${locale}/enseignants` }));
    ...
    <SectionReveal className="bg-white">
      <div className="container mx-auto px-4 py-10">
        ...
        <div className="text-center">
          <h2 className="text-2xl md:text-3xl font-extrabold uppercase text-[#25282E]">Corps Enseignant</h2>
          <p className="text-neutral-500 text-sm md:text-base mt-1">Nos professeurs et formateurs</p>
            ...
            <div key={p.id} className="text-center">
              <figure className="group relative overflow-hidden rounded bg-white border border-neutral-200">
                <div className="relative aspect-[4/3] overflow-hidden">
                  <Image src={p.image} alt={p.name} fill sizes="(min-width:1024px) 16vw, (min-width:640px) 33vw, 50vw" className="object-cover transition-transform duration-300 group-hover:scale-105 group-focus-within:scale-105" />
                  {/* overlay & CTA */}
                  ...
                  <figcaption className="absolute left-0 right-0 bottom-0 p-3 text-white translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100 transition-all duration-300">
                    <Link href={p.href} className="inline-flex items-center bg-white text-[#0F172A] rounded px-3 py-1 text-xs font-semibold hover:bg-white/90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white" aria-label={`Voir le profil de ${p.name}`}>
                      Voir le profil
                    </Link>
                  </figcaption>
                </div>
        ...
        <div className="mt-6 text-center">
          <Link href={`/${locale}/enseignants`} className="inline-flex items-center gap-2 bg-white border-2 border-[#2E9B51] text-[#2E9B51] rounded px-3 py-2 text-sm font-semibold hover:bg-[#2E9B51]/5">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 12l-10-5-10 5 10 5 10-5z"/><path d="M6 12v5c2.5 2 9.5 2 12 0v-5"/></svg>
